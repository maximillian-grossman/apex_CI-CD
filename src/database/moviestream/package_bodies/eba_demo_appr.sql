create or replace package body eba_demo_appr as
    ...
    begin
        for j in (
            ...
            where
                    task_def_static_id = upper(p_task_def_static_id)
                and ( job_codes is null
                      ...
                      or ( ':'
                           ...
                           || ':' like '%:'
                                       || upper(p_job)
                                       || ':%' ) )
                and ( min_salary <= p_proposed_sal
                      or min_salary is null )
                and participant_role = p_role
                and ( participant_role = 'ADMIN'
                      or username != l_current_user )
            order by
                job_codes nulls last
        ) loop
            ...
            if j.job_codes is not null
               or l_include_fallback_entries then
    ...
    begin
        return get_participant_for_role(p_task_def_static_id, p_job, p_proposed_sal, 'APPROVER');
    ...
    begin
        return get_participant_for_role(p_task_def_static_id, p_job, p_proposed_sal, 'ADMIN');
    end get_admin_for;
    ...
    begin
        for j in (
            select
                null
            from
                apex_task_participants tp
                left join apex_tasks             t on t.task_id = tp.task_id
            where
                    tp.participant = l_app_user
                and t.state_code in ( 'UNASSIGNED', 'ASSIGNED' )
                and tp.participant_type = 'POTENTIAL_OWNER'
                and ( t.initiator is null
                      or t.initiator != l_app_user )
            fetch first row only
        ) loop
            apex_debug.info('### Returning true for user %s ' || 'having open approvals tasks', l_app_user);
            return true;
        end loop;
    ...
    begin
        for j in (
            ...
            apex_debug.info('### Returning true for user %s ' || 'having open admin tasks', l_app_user);
            return true;
        end loop;
    ...
    begin
        ...
        p_admin := get_admin_for(p_task_def_static_id, p_empno, l_job, p_proposed_sal);
        p_approver := get_approver_for(p_task_def_static_id, p_empno, l_job, p_proposed_sal);
    ...
    begin
        ...
        select
            listagg(participant, ', ') within group(
            order by
                participant
            )
        into l_ret
        ...
        if p_vp_username is null then
            ...
            if p_vp_username != 'NONE' then
                ...
                raise_application_error(-20001,
                                        apex_string.format('%s is not a valid employee name.', p_vp_username));
            end if;
        end if;
    exception
        when no_data_found then
    ...
    function mask_for_date_page_item (
        p_item_name varchar2
    ) return varchar2 is
        l_mask varchar2(200);
    begin
        select
            coalesce(format_mask,
                     v(c_app_date_fmt))
        into l_mask
        from
            apex_application_page_items
        where
                application_id = v(c_app_id)
    ...
    --
    function rejection_delay_until_time return date is
    begin
    ...
    begin
        ...
        for j in (
            select
                workflow_id as laptop_request_workflow_id
            from
                eba_demo_appr_laptop_requests
            where
                    id = p_laptop_request_id
            ...
            for k in (
                select
                    workflow_id as procurement_workflow_id
                from
                    apex_workflows
                where
                        workflow_def_static_id = 'LAPTOP_PROCUREMENT'
                    and parent_workflow_id = j.laptop_request_workflow_id
            ) loop
    ...
    begin
        ...
        from
            eba_demo_appr_emp  e
            left outer join eba_demo_appr_dept d on d.deptno = e.deptno
        where
            d.dname = p_dname;
    ...
    begin
        p_additional_participants(l_idx) := apex_approval.t_task_participant_change(
            apex_approval.t_task_participant(apex_approval.c_task_business_admin, apex_approval.c_task_identity_type_user, p_old),
            apex_approval.t_task_participant(apex_approval.c_task_business_admin, apex_approval.c_task_identity_type_user, p_new),
            coalesce(p_reason,
                     apex_string.format('%s temporarily covering %s business admin dutites', p_new, p_taskdef_name))
        );
    ...
    begin
        p_additional_participants(l_idx) := apex_approval.t_task_participant_change(
            apex_approval.t_task_participant(apex_approval.c_task_potential_owner, apex_approval.c_task_identity_type_user, p_old),
            apex_approval.t_task_participant(apex_approval.c_task_potential_owner, apex_approval.c_task_identity_type_user, p_new),
            coalesce(p_reason,
                     apex_string.format(
                case
                    when p_wildcard then
                        '%s helping to cover %s'
                    else '%s covering %s for vacationing %s'
                end, p_new, p_taskdef_name, p_old))
        );
    end add_participant;
    --
    ...
    begin
        if l_temp_business_admin is not null then
            for j in 1..p_participants.count loop
                if
                    p_participants(j).type = apex_approval.c_task_business_admin
                    and p_participants(j).identity = apex_approval.c_task_identity_type_user
                then
                    add_business_admin(
    ...
    begin
        for j in 1..p_participants.count loop
            if
                p_participants(j).type = apex_approval.c_task_potential_owner
                and p_participants(j).identity = apex_approval.c_task_identity_type_user
            then
                apex_string.push(l_ret,
                                 p_participants(j).value);
            end if;
        end loop;
    ...
    begin
        ...
        from
            apex_appl_taskdefs
        where
                static_id = p_task_def_static_id
                    ...
                    ( original_user is null
                      or original_user in (
                        select
                            column_value
                        from
                            table ( l_participants )
                    ) )
                    and instr(':'
                              || task_def_ids
                              || ':', ':'
                                      || p_param.task_def_static_id
                                      || ':') > 0
    ...
    begin
        ...
        if l_participants.count > 0 then
            ...
            for j in (
                select
                    column_value as substitute_user
                from
                    apex_string.split(
                        apex_app_setting.get_value('EMP_APPRAISAL_EXTRA_VP_REVIEWERS'),
                        ':'
                    )
            ) loop
                add_participant(
    ...
    begin
        for j in (
            ...
            where
                    laptop_type = upper(p_laptop_type)
                and amount > 0
        ) loop
            return true;
        end loop;
    ...
    begin
        ...
        update eba_demo_appr_laptop_requests
        set
            order_id = trunc(dbms_random.value(1111111, 9999999)),
...
-- sqlcl_snapshot {"hash":"ca446055c775ae3b0ea48d31d9d1a95b1068f2c2","type":"PACKAGE_BODY","name":"EBA_DEMO_APPR","schemaName":"MOVIESTREAM","sxml":""}