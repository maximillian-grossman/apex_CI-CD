create or replace package body eba_demo_ig_text_pkg is
    ...
    begin
        execute immediate q'#create index eba_demo_ig_text_ftx on eba_demo_ig_people (name)
   indextype is ctxsys.context parameters ( 'section group  EBA_DEMO_IG_SG_PREF
    ...
    function convert_text_query (
        ...
        begin
            for i in 1..l_tokens.count loop
                l_clean_token := lower(regexp_replace(
                    l_tokens(i),
                    '[<>{}/()*%&!$?.:,;\+#]',
                    ''
                ));
    ...
    begin
        if substr(p_enduser_query, 1, 8) = 'ORATEXT:' then
            ...
            l_textquery := replace(l_textquery,
                                   '#NORMAL_AND#',
                                   generate_query('NORMAL', 'AND'));
            l_textquery := replace(l_textquery,
                                   '#FUZZY_AND#',
                                   generate_query('FUZZY', 'AND'));
            return l_textquery;
        end if;
...
-- sqlcl_snapshot {"hash":"bdccbcf4024090dc4f1c0c223a9506c4c6716454","type":"PACKAGE_BODY","name":"EBA_DEMO_IG_TEXT_PKG","schemaName":"MOVIESTREAM","sxml":""}