    ...
    begin
        for i in (
            select
                ...
                https_host
            from
                eba_restdemo_sample_urls
        ) loop
            update eba_restdemo_sample_urls
            set
                last_status = test_url(i.url, i.https_host)
            where
                name = i.name;
        ...
        exception
            when others then
                if sqlcode = -24247 then
                    l_result := 'ERROR_ACL';
                elsif sqlcode = -29024 then
                    l_result := 'ERROR_CERT';
                elsif sqlcode = -12535 then
                    l_result := 'ERROR_NETWORK';
                else
                    l_result := 'ERROR_OTHER_' || sqlerrm;
                end if;
        end;
...
-- sqlcl_snapshot {"hash":"e979bf5917d20473c50a17925a77a7273c892190","type":"PACKAGE_BODY","name":"EBA_RESTDEMO_SAMPLE_PKG","schemaName":"MOVIESTREAM","sxml":""}