create or replace function gen_ai_chat_legislation_db (
...
-- if executing in sqldeveloper app_session is not set, otherwise it is set in APEX
    ...
    from
        conversations
    where
        app_session = sys_context('userenv', 'sessionid');

    if v_session = 0 then
        chathistory_pkg.prc_add_conversation(user,
                                             systimestamp,
...
-- sqlcl_snapshot {"hash":"05c58d6eadac96e37342bfd6819158724c523e08","type":"FUNCTION","name":"GEN_AI_CHAT_LEGISLATION_DB","schemaName":"MOVIESTREAM","sxml":""}