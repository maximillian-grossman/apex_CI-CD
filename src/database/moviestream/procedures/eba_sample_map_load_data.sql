...
begin
    ...
    if l_states_json is not null then
        ...
        insert into eba_sample_map_states (
            id,
            name,
            state_code,
            land_area,
            ...
            (
                select
                    ...
                    geometry
                from
                    json_table ( l_states_json format json, '$[*]'
                        columns (
                            id number path '$.id',
        ...
        insert into eba_sample_map_simple_states (
            id,
            name,
            state_code,
            land_area,
            ...
            (
                select
                    ...
                    land_area,
                    water_area,
                    mdsys.sdo_util.simplify(geometry, 75)
                from
        ...
        insert into eba_sample_map_airports (
            ...
            (
                select
                    ...
                    city,
                    state_name,
                    to_char(to_date(activation_date, 'YYYY-MM-DD"T"HH24:MI:SS"Z"'), 'MON-RR'),
                    to_date(activation_date, 'YYYY-MM-DD"T"HH24:MI:SS"Z"'),
                    elevation,
                    dist_city_to_airport,
                    land_area_covered,
                    commercial_ops,
                    ...
                    geometry
                from
                    json_table ( l_airports_json format json, '$[*]'
                        columns (
...
-- sqlcl_snapshot {"hash":"d585f2626cba496f4a7a1c6a5318ceb1ac6ca23c","type":"PROCEDURE","name":"EBA_SAMPLE_MAP_LOAD_DATA","schemaName":"MOVIESTREAM","sxml":""}